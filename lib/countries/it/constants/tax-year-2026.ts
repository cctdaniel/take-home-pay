...
export const IT_TAX_CONFIG = {
  ...
  taxCredit: (grossSalary: number) =>
    grossSalary <= 15_000
      ? 1_955
      : grossSalary <= 28_000
        ? Math.max(0, 1_910 + 1_190 * ((28_000 - grossSalary) / 13_000))
        : grossSalary <= 50_000
          ? Math.max(0, 1_910 * ((50_000 - grossSalary) / 22_000))
          : 0,
  brackets: [
  ...
  assumptions: [
    ...
    "Exact regional addizionale, municipal addizionale, spouse/dependent deductions, bonus/exoneration rules, fringe benefits, severance pay, and employer-only costs are outside the model.",
  ],
