...
export function buildCountryComparison({
  ...
  const zaInputs: ZACalculatorInputs = { ...defaultInputs, grossSalary: grossLocal, payFrequency };
  const result = calculateNetSalary(zaInputs);
  const assumptions = buildAssumptionsSummary(country, inputs, false);
  assumptions.push("Progressive 18-45% tax + 1% UIF");
