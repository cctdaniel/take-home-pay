...
export function CLResultBreakdown({
  result,
  grossSalary,
  currency,
}: CountryResultBreakdownProps) {
  const { taxes, breakdown } = result;
  if (!("type" in taxes) || taxes.type !== "CL" || breakdown.type !== "CL") {
    return null;
  }
  ...
  return (
    <>
      ...
      <DeductionRow
        label={`AFP Pension (${formatPercentage(breakdown.mandatoryContributions.afpRate)})`}
        amount={taxes.afp}
      ...
      />
      <DeductionRow
        label={`Unemployment (${formatPercentage(breakdown.mandatoryContributions.unemploymentRate)})`}
